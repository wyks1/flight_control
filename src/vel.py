...
class VelocityBody:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/t265/odom/sample", Odometry, self.posCb)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        last_pos_x.append(cnt.current_position.pose.pose.position.x)
        last_pos_y.append(cnt.current_position.pose.pose.position.y)
        last_pos_z.append(cnt.current_position.pose.pose.position.z)
        print("x:", cnt.current_position.pose.pose.position.x)
        if(len(last_pos_x) > 5):
            last_pos_x.pop(0)
            last_pos_y.pop(0)
            last_pos_z.pop(0)
        ...
        vel_x_cur = (pose_est_x - cnt.last_position.pose.pose.position.x) / deltat
        vel_y_cur = (pose_est_y - cnt.last_position.pose.pose.position.y) / deltat
        ...
        cnt.last_velocity.pose.pose.position.x = vel_x_cur
        cnt.last_velocity.pose.pose.position.y = vel_y_cur
        cnt.last_velocity.pose.pose.position.z = vel_z_cur
        cnt.last_velocity_est = outer_velocity
        cnt.last_position.pose.pose.position.x = pose_est_x
        cnt.last_position.pose.pose.position.y = pose_est_y
